...
GO
EXECUTE sp_addextendedproperty @name = N'UpdateQuery', @value = 'MERGE  Paga_EDW.[Classification].[DimProcessStatus] AS Target
			USING 
			(
				SELECT
						x.*
				...
				(
					SELECT
						...
						stg.*
					FROM Paga_Staging.Updates.Classification_DimProcessStatus AS stg
				) as x
				WHERE x.rn = 1
...
GO
CREATE NONCLUSTERED INDEX [ix_DimProcessStatus_ChangeVersion]
    ON [Classification].[DimProcessStatus]([SYS_CHANGE_VERSION] ASC);
