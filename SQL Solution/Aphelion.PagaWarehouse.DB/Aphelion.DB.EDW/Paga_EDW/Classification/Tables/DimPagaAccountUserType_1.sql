...
GO
EXECUTE sp_addextendedproperty @name = N'UpdateQuery', @value = 'MERGE  Paga_EDW.[Classification].[DimPagaAccountUserType] AS Target
			USING 
			(
				SELECT
						x.*
				...
				(
					SELECT
						...
						stg.*
					FROM Paga_Staging.Updates.Classification_DimPagaAccountUserType AS stg
				) as x
				WHERE x.rn = 1
...
GO
CREATE NONCLUSTERED INDEX [ix_DimPagaAccountUserType_ChangeVersion]
    ON [Classification].[DimPagaAccountUserType]([SYS_CHANGE_VERSION] ASC);
