...
GO
EXECUTE sp_addextendedproperty @name = N'UpdateQuery', @value = 'MERGE  Paga_EDW.[Classification].[DimBusinessType] AS Target
			USING 
			(
				SELECT
						x.*
				...
				(
					SELECT
						...
						stg.*
					FROM Paga_Staging.Updates.Classification_DimBusinessType AS stg
				) as x
				WHERE x.rn = 1
...
GO
CREATE NONCLUSTERED INDEX [ix_DimBusinessType_ChangeVersion]
    ON [Classification].[DimBusinessType]([SYS_CHANGE_VERSION] ASC);
