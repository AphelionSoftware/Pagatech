...
GO
EXECUTE sp_addextendedproperty @name = N'UpdateQuery', @value = 'MERGE  Paga_EDW.[Location].[DimLocalGovernmentArea] AS Target
			USING 
			(
				SELECT
						x.*
				...
				(
					SELECT
						...
						stg.*
					FROM Paga_Staging.Updates.Location_DimLocalGovernmentArea AS stg
				) as x
				WHERE x.rn = 1
			...
			THEN
				UPDATE SET 
				Target.SourceKey = Source.SourceKey,Target.Name = Source.Name,Target.Code = Source.Code,Target.DimRegionID = Source.DimRegionID,Target.SYS_CHANGE_VERSION = Source.SYS_CHANGE_VERSION,Target.SYS_CHANGE_OPERATION = Source.SYS_CHANGE_OPERATION
			WHEN NOT MATCHED BY TARGET
			THEN
				INSERT 
				(
					SourceKey,Name,Code,DimRegionID,SYS_CHANGE_VERSION,SYS_CHANGE_OPERATION
				)
			VALUES 
			(
				Source.SourceKey,Source.Name,Source.Code,Source.DimRegionID,Source.SYS_CHANGE_VERSION,Source.SYS_CHANGE_OPERATION
			);', @level0type = N'SCHEMA', @level0name = N'Location', @level1type = N'TABLE', @level1name = N'DimLocalGovernmentArea';
...
GO
CREATE UNIQUE NONCLUSTERED INDEX [ix_DimLocalGovernmentArea_SourceKey]
    ON [Location].[DimLocalGovernmentArea]([SourceKey] ASC);


GO
CREATE NONCLUSTERED INDEX [ix_DimLocalGovernmentArea_ChangeVersion]
    ON [Location].[DimLocalGovernmentArea]([SYS_CHANGE_VERSION] ASC);
