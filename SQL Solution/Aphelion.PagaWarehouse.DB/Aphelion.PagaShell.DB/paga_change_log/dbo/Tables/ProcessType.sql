...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			ProcessTypeId
		FROM CHANGETABLE(CHANGES [dbo].[ProcessType] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ProcessType';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_ProcessType]
    ON [dbo].[ProcessType]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [ProcessTypeId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					ProcessTypeId,
					ROW_NUMBER() OVER (ORDER BY ProcessTypeId) as rn
				FROM [dbo].[ProcessType]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [ProcessType], (ProcessTypeId), (stream.ProcessTypeId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ProcessType';
