...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			FinancialAccountId
		FROM CHANGETABLE(CHANGES [dbo].[FinancialAccount] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'FinancialAccount';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_FinancialAccount]
    ON [dbo].[FinancialAccount]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [FinancialAccountId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					FinancialAccountId,
					ROW_NUMBER() OVER (ORDER BY FinancialAccountId) as rn
				FROM [dbo].[FinancialAccount]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [FinancialAccount], (FinancialAccountId), (stream.FinancialAccountId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'FinancialAccount';
