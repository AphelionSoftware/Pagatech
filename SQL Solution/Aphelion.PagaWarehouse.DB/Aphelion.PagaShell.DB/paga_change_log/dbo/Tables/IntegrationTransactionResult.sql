...
CREATE UNIQUE CLUSTERED INDEX [ix_IntegrationTransactionResult]
    ON [dbo].[IntegrationTransactionResult]([IntegrationTransactionResultId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			IntegrationTransactionResultId
		FROM CHANGETABLE(CHANGES [dbo].[IntegrationTransactionResult],0)  AS change_log', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'IntegrationTransactionResult';
