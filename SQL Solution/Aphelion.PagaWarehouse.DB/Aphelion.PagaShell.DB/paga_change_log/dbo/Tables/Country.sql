...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			CountryId
		FROM CHANGETABLE(CHANGES [dbo].[Country] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Country';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_Country]
    ON [dbo].[Country]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [CountryId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [Country], (CountryId), (stream.CountryId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Country';
