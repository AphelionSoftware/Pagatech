...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			PagaAccountUserId
		FROM CHANGETABLE(CHANGES [dbo].[PagaAccountUser] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'PagaAccountUser';
...
CREATE UNIQUE CLUSTERED INDEX [ix_PagaAccountUser]
    ON [dbo].[PagaAccountUser]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [PagaAccountUserId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
