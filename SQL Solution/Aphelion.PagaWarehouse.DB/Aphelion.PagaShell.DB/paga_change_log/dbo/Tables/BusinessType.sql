...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			BusinessTypeId
		FROM CHANGETABLE(CHANGES [dbo].[BusinessType] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'BusinessType';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_BusinessType]
    ON [dbo].[BusinessType]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [BusinessTypeId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					BusinessTypeId,
					ROW_NUMBER() OVER (ORDER BY BusinessTypeId) as rn
				FROM [dbo].[BusinessType]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [BusinessType], (BusinessTypeId), (stream.BusinessTypeId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'BusinessType';
