...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			OrganizationUnitUserId
		FROM CHANGETABLE(CHANGES [dbo].[OrganizationUnitUser] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'OrganizationUnitUser';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_OrganizationUnitUser]
    ON [dbo].[OrganizationUnitUser]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [OrganizationUnitUserId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					OrganizationUnitUserId,
					ROW_NUMBER() OVER (ORDER BY OrganizationUnitUserId) as rn
				FROM [dbo].[OrganizationUnitUser]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [OrganizationUnitUser], (OrganizationUnitUserId), (stream.OrganizationUnitUserId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'OrganizationUnitUser';
