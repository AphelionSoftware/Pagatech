...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		FROM CHANGETABLE(CHANGES [dbo].[IntegrationTransaction] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'IntegrationTransaction';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_IntegrationTransaction]
    ON [dbo].[IntegrationTransaction]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [IntegrationTransactionId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					IntegrationTransactionId,
					ROW_NUMBER() OVER (ORDER BY IntegrationTransactionId) as rn
				FROM [dbo].[IntegrationTransaction]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [IntegrationTransaction], (IntegrationTransactionId), (stream.IntegrationTransactionId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'IntegrationTransaction';
