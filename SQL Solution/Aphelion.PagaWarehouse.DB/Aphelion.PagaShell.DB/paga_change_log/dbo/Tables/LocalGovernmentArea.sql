...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		FROM CHANGETABLE(CHANGES [dbo].[LocalGovernmentArea] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'LocalGovernmentArea';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_LocalGovernmentArea]
    ON [dbo].[LocalGovernmentArea]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [LocalGovernmentAreaId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					LocalGovernmentAreaId,
					ROW_NUMBER() OVER (ORDER BY LocalGovernmentAreaId) as rn
				FROM [dbo].[LocalGovernmentArea]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [LocalGovernmentArea], (LocalGovernmentAreaId), (stream.LocalGovernmentAreaId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'LocalGovernmentArea';
