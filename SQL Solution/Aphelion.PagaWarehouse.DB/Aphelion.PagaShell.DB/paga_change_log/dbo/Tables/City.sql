...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			CityId
		FROM CHANGETABLE(CHANGES [dbo].[City] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'City';
...
CREATE UNIQUE CLUSTERED INDEX [ix_City]
    ON [dbo].[City]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [CityId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					CityId,
					ROW_NUMBER() OVER (ORDER BY CityId) as rn
				FROM [dbo].[City]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [City], (CityId), (stream.CityId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'City';
