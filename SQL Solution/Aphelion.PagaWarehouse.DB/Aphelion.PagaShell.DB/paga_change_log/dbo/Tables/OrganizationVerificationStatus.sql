...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		FROM CHANGETABLE(CHANGES [dbo].[OrganizationVerificationStatus] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'OrganizationVerificationStatus';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_OrganizationVerificationStatus]
    ON [dbo].[OrganizationVerificationStatus]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [OrganizationVerificationStatusId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					OrganizationVerificationStatusId,
					ROW_NUMBER() OVER (ORDER BY OrganizationVerificationStatusId) as rn
				FROM [dbo].[OrganizationVerificationStatus]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [OrganizationVerificationStatus], (OrganizationVerificationStatusId), (stream.OrganizationVerificationStatusId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'OrganizationVerificationStatus';
