...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			UserRoleId
		FROM CHANGETABLE(CHANGES [dbo].[UserRole] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'UserRole';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_UserRole]
    ON [dbo].[UserRole]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [UserRoleId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					UserRoleId,
					ROW_NUMBER() OVER (ORDER BY UserRoleId) as rn
				FROM [dbo].[UserRole]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [UserRole], (UserRoleId), (stream.UserRoleId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'UserRole';
