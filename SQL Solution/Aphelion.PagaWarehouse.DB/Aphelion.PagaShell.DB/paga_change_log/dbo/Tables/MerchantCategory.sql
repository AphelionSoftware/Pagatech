...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			MerchantCategoryId
		FROM CHANGETABLE(CHANGES [dbo].[MerchantCategory] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'MerchantCategory';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_MerchantCategory]
    ON [dbo].[MerchantCategory]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [MerchantCategoryId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					MerchantCategoryId,
					ROW_NUMBER() OVER (ORDER BY MerchantCategoryId) as rn
				FROM [dbo].[MerchantCategory]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [MerchantCategory], (MerchantCategoryId), (stream.MerchantCategoryId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'MerchantCategory';
