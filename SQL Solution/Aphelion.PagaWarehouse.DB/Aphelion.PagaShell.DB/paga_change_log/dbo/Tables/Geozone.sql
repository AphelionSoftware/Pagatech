...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			GeozoneId
		FROM CHANGETABLE(CHANGES [dbo].[Geozone] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Geozone';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_Geozone]
    ON [dbo].[Geozone]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [GeozoneId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					GeozoneId,
					ROW_NUMBER() OVER (ORDER BY GeozoneId) as rn
				FROM [dbo].[Geozone]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [Geozone], (GeozoneId), (stream.GeozoneId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Geozone';
