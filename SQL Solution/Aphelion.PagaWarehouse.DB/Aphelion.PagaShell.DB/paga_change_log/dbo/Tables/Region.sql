...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			RegionId
		FROM CHANGETABLE(CHANGES [dbo].[Region] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Region';
...
CREATE UNIQUE CLUSTERED INDEX [ix_Region]
    ON [dbo].[Region]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [RegionId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
