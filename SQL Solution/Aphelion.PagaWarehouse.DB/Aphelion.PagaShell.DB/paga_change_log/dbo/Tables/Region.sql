...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_Region]
    ON [dbo].[Region]([RegionId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					RegionId,
					ROW_NUMBER() OVER (ORDER BY RegionId) as rn
				FROM [dbo].[Region]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION Region, (RegionId), (stream.RegionId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Region';
