...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		FROM CHANGETABLE(CHANGES [dbo].[FinancialTransactionSubType] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'FinancialTransactionSubType';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_FinancialTransactionSubType]
    ON [dbo].[FinancialTransactionSubType]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [FinancialTransactionSubTypeId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					FinancialTransactionSubTypeId,
					ROW_NUMBER() OVER (ORDER BY FinancialTransactionSubTypeId) as rn
				FROM [dbo].[FinancialTransactionSubType]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [FinancialTransactionSubType], (FinancialTransactionSubTypeId), (stream.FinancialTransactionSubTypeId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'FinancialTransactionSubType';
