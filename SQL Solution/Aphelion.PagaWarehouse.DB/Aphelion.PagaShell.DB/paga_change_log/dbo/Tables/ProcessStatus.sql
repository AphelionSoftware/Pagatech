...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			ProcessStatusId
		FROM CHANGETABLE(CHANGES [dbo].[ProcessStatus] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ProcessStatus';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_ProcessStatus]
    ON [dbo].[ProcessStatus]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [ProcessStatusId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					ProcessStatusId,
					ROW_NUMBER() OVER (ORDER BY ProcessStatusId) as rn
				FROM [dbo].[ProcessStatus]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [ProcessStatus], (ProcessStatusId), (stream.ProcessStatusId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ProcessStatus';
