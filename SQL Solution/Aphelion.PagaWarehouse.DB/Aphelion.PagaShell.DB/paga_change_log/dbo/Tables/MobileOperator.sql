...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			MobileOperatorId
		FROM CHANGETABLE(CHANGES [dbo].[MobileOperator] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'MobileOperator';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_MobileOperator]
    ON [dbo].[MobileOperator]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [MobileOperatorId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					MobileOperatorId,
					ROW_NUMBER() OVER (ORDER BY MobileOperatorId) as rn
				FROM [dbo].[MobileOperator]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [MobileOperator], (MobileOperatorId), (stream.MobileOperatorId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'MobileOperator';
