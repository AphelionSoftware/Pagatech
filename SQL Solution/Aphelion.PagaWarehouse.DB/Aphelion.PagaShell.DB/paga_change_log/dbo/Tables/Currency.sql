...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			CurrencyId
		FROM CHANGETABLE(CHANGES [dbo].[Currency] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Currency';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_Currency]
    ON [dbo].[Currency]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [CurrencyId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [Currency], (CurrencyId), (stream.CurrencyId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Currency';
