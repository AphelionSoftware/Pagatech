...
EXECUTE sp_addextendedproperty @name = N'BaseQuery', @value = N'
		...
		SELECT 
			...
			ProcessTypeGroupId
		FROM CHANGETABLE(CHANGES [dbo].[ProcessTypeGroup] ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ProcessTypeGroup';
...
GO
CREATE UNIQUE CLUSTERED INDEX [ix_ProcessTypeGroup]
    ON [dbo].[ProcessTypeGroup]([as_of_change_version] ASC, [SYS_CHANGE_OPERATION] ASC, [ProcessTypeGroupId] ASC, [row_id] ASC) WITH (DATA_COMPRESSION = PAGE);
...
EXECUTE sp_addextendedproperty @name = N'Stream3', @value = N'
		...
		(
			...
			( 
				SELECT 
					...
					ProcessTypeGroupId,
					ROW_NUMBER() OVER (ORDER BY ProcessTypeGroupId) as rn
				FROM [dbo].[ProcessTypeGroup]  AS bq
			) AS bq
		...
		CROSS APPLY 
			CHANGETABLE (VERSION [ProcessTypeGroup], (ProcessTypeGroupId), (stream.ProcessTypeGroupId)) AS c
		OPTION (MAXDOP 1)', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ProcessTypeGroup';
