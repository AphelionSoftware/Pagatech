...
/* Populate Your Dimension Table with values*/
	...
	SELECT
		...
		CONVERT (char(8),@CurrentDate,112) as DimDateID,
		@CurrentDate AS Date,
...
/*Update HOLIDAY Field of USA In dimension*/
	...
	DateID int, Week TINYINT, YEAR CHAR(4), DAY CHAR(2))
		...
		FROM Shared.DimDate DT
			JOIN @Holidays HL ON (HL.DateID + 1) = DT.DimDateID
		WHERE
			[Week] = 1
	END
...
--ALTER TABLE shared.[DimDate] ADD
...
DECLARE
	@dtFiscalYearStart SMALLDATETIME = 'January 01, 2008',
	@FiscalYear INT = 2008,
	@LastYear INT = 2018,
	@FirstLeapYearInPeriod INT = 2008
...
FROM @tb a
	INNER JOIN Shared.DimDate b ON a.PeriodDate = b.[FullDate]

/********************************************************************************************/

SELECT * FROM Shared.DimDate 
