...
(
	...
	(
		SELECT
			WithdrawalCode=MAX(CASE WHEN pd.Name='WithdrawalCode' THEN CONVERT(VARCHAR(200),CONVERT(VARCHAR(200),pd.value)) END),
			...
			DimApprovedByUserSourceKey= MAX(CASE WHEN pd.Name='approvedByUserId' THEN CONVERT(VARCHAR(200),pd.value) END),
			DimCancellationApprovedByUserSourceKey=MAX(CASE WHEN pd.Name='cancelledByUserId' THEN CONVERT(VARCHAR(200),pd.value) END)
